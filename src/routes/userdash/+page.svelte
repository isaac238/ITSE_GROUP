<script>
	...
	const navState = writable("Dashboard");
	
	function updateNavState(state) { //Sets nav state for single page navigation
		navState.set(state);
	}
...
<div class="w-screen h-screen flex flex-col items-center bg-transparent">
  <ul class="menu menu-horizontal rounded-xl my-2 p-0 h-fit">
	...
	<div class="md:hidden flex snap-x snap-mandatory overflow-x-auto p-4 space-x-4 bg-neutral w-screen flex-grow">
		
		<MobileItem title="Component Title" subtitle="Subtitle" description="description" />

	<!--
		Implement component rendering as such: 

		Replace the workout item with items received from the database. Adjust fields accordingly.

		{#each workout as item}
			<MobileItem title={item.title} subtitle={item.subtitle} description={item.description} />
		{/each} -->
 ...
 <!-- Same implementation here, just feed the values in from the same source, will populate both UIs. -->
