<script>
	...
	const fooditemIsValid = () => {
		const hasEmpty = Object.values(newFoodModalState.fooditem).some((value) => value === "");
		return !hasEmpty;
	}
...
<div class="modal-box bg-gradient-to-b from-gray-800 to-gray-950">
	<h2 class="text-xl text-white">New: <span class="font-semibold">Food</span></h2>
	<div class="flex flex-col gap-2 p-5">
	<form on:submit|preventDefault={handleSubmit} class="flex flex-col gap-4">
		{#if newFoodModalState.step == 0}
			<label for="foodType" class="text-white">Choose a Food Type</label>
			<div>
				<LabeledInput bind:value={newFoodModalState.foodListFilter} name="foodType" type="text" placeholder="Food Name"  required={true}/>
				{#if showFoodList}
				<div class=" flex flex-col items-center mt-4 gap-2 overflow-y-scroll min-h-fit max-h-28">
					{#each allCurrentFoodItems as fooditem}
						{#if fooditem.name.toLowerCase().includes(newFoodModalState.foodListFilter.toLowerCase())}
							<button on:click={() => {
								newFoodModalState.fooditem = fooditem;
								newFoodModalState.foodListFilter = fooditem.name;
							}} type="button" class="block w-full bg-base-100 rounded-lg p-2">{fooditem.name}</button>
						{/if}
					{/each}
				</div>
				{/if}
			...
			<h1>Or</h1>
			<button type="button" class="btn btn-primary bg-white text-black hover:text-white hover:bg-black rounded-lg cursor-pointer border-none text-md" on:click={() => createNewFoodClicked() }>Add a new Food Type</button>
		{/if}

		{#if createNewFood && newFoodModalState.step == 1}
			<LabeledInput bind:value={newFoodModalState.fooditem.name} name="Food Name" type="text" placeholder="Food Name" required={true}/>
			<h1 class="font-bold">Macros per 100g</h1>
			<LabeledInput bind:value={newFoodModalState.fooditem.calories_in_g} name="Calories (Kcal)" type="number" min="0" step="1" placeholder="Calories" required={true}/>
			<LabeledInput bind:value={newFoodModalState.fooditem.protein_in_g} name="Protein (g)" type="number" min="0" step="0.1" placeholder="Protein" required={true}/>
			<LabeledInput bind:value={newFoodModalState.fooditem.carbs_in_g} name="Carbs (g)" type="number" min="0" step="0.1" placeholder="Carbs" required={true}/>
			<LabeledInput bind:value={newFoodModalState.fooditem.fats_in_g} name="Fats (g)" type="number" min="0" step="0.1" placeholder="Fats" required={true}/>
			<LabeledInput bind:value={newFoodModalState.fooditem.sugar_in_g} name="Sugar (g)" type="number" min="0" step="0.1" placeholder="Sugar" required={true}/>
		{/if}

		{#if (newFoodModalState.step == 1 && !createNewFood) || (newFoodModalState.step == 2 && createNewFood)}
			<LabeledInput bind:value={newFoodModalState.portion} name="Portion (g)" type="number" min="0" step="1" placeholder="Portion" required={true}/>
		{/if}
